...
options.includeAttributeNameFormat = (typeof options.includeAttributeNameFormat !== 'undefined') ? options.includeAttributeNameFormat : false;
...
var uuid = (options.uid || utils.uid(32));
// This is for populating saml2p:Response tag attributes
doc.documentElement.setAttribute('ID', '_' + uuid);
doc.documentElement.setAttribute('Destination', options.destination);
doc.documentElement.setAttribute('InResponseTo', options.responseTo);
if (options.issuer) {
var issuerObjects = doc.documentElement.getElementsByTagName('saml2:Issuer');
// Multiple issuer statements may be present (1.e for Response as well as assertion tag)
for(var i=0; i < issuerObjects.length; i++){
  issuerObjects[i].textContent = options.issuer;
}

}
var now = moment.utc();
var issueInstant = now.format('YYYY-MM-DDTHH:mm:ss.SSS[Z]');
doc.documentElement.setAttribute('IssueInstant', issueInstant);

// For populating saml2:Assertion tag attributes
var samlassertTag = doc.getElementsByTagName('saml2:Assertion')[0];
samlassertTag.setAttribute('ID','_' + uuid);
samlassertTag.setAttribute('IssueInstant', issueInstant); 
...
var audienceRestriction = doc.createElementNS(NAMESPACE, 'saml2:AudienceRestriction');
...
var statement = doc.createElementNS(NAMESPACE, 'saml2:AttributeStatement');
...
// Include the attribute tags inside saml2:Assertion tag
doc.getElementsByTagName('saml2:Assertion')[0].appendChild(statement);
...
var samlAuthStatementTag = doc.getElementsByTagName('saml2:AuthnStatement')[0];
samlAuthStatementTag.setAttribute('AuthnInstant', now.format('YYYY-MM-DDTHH:mm:ss.SSS[Z]'));
samlAuthStatementTag.setAttribute('SessionNotOnOrAfter', now.clone().add(options.lifetimeInSeconds, 'seconds').format('YYYY-MM-DDTHH:mm:ss.SSS[Z]'));
if (options.sessionIndex) {
samlAuthStatementTag.setAttribute('SessionIndex', options.sessionIndex);
}  
...
if (options.nameQualifier) {
nameID.setAttribute('NameQualifier', options.nameQualifier);
}
